...
function LoanDetails() {
		...
		await axios.post(`${url_bdccb}/recov/calculate_prn_intt_amt`, creds, {
			...
			.then((res) => {
				...
				if (res?.data?.success) {
					...
					// setLoanDetails(res?.data?.data || [])
					const members = (res?.data?.data || []).map((item, index) => ({
						...item,
						disb_amt: formik.values.members[index]?.disb_amt || item.disb_amt,
	...
	const handleTransactionDateChange = async (e) => {
		...
		if (trnDate && loanDetails[0]?.member_list?.[0]?.ccb_loan_id) {
			const creds = {
				ccb_loan_id: loanDetails[0]?.member_list[0]?.ccb_loan_id,
				trans_date: trnDate
			};
					...
					<>
						...
						{formik.values.members?.length > 0 && (
							...
							dark:text-gray-100"> Member Name</label>
									...
									<div>
										<label for="members.0.loan_id" class="block mb-0 text-sm capitalize font-bold text-slate-800
							dark:text-gray-100"> Disb. Amount</label>
									</div>
							...
							dark:text-gray-100"> Amount</label>
									...
									<div>
										<label for="members.0.loan_id" class="block mb-0 text-sm capitalize font-bold text-slate-800
							dark:text-gray-100">  Outst. Amount</label>
									</div>

									<div>
										<label for="members.0.loan_id" class="block mb-0 text-sm capitalize font-bold text-slate-800
							dark:text-gray-100">  Calc. Interest</label>
									</div>

									<div>
										<label for="members.0.loan_id" class="block mb-0 text-sm capitalize font-bold text-slate-800
							dark:text-gray-100">  Prin. Recovery</label>
									</div>

									<div>
										<label for="members.0.loan_id" class="block mb-0 text-sm capitalize font-bold text-slate-800
							dark:text-gray-100">  Int. Recovery</label>
									</div>
								...
								{formik.values.members.map((member, index) => (
									...
									<div key={index} className="grid grid-cols-8 gap-5 mt-0">
										...
										<div>
											<TDInputTemplateBr
												placeholder="Disb. Amount"
												type="number"
												name={`members.${index}.disb_amt`}
												formControlName={member.disb_amt || ""}
												disabled={true}
												mode={1}
											/>
										</div>
