...
function RecoverySubmitStatusGroupBranchBM_BDCCB() {
	...
	const handleExportMembers = (loans) => {
		...
		const fileName = `GroupRecovery_${recoveryStatus}_Members_${new Date().toISOString().slice(0, 10)}.xlsx`;
	...
	const fetchSearchedGroups = async () => {
		...
		const creds = {
			...
			from_dt: recoveryStatus == 'A' ? fromDate : transactionDate,
			to_dt: recoveryStatus == 'A' ? toDate : transactionDate,
			approval_status: recoveryStatus,
			branch_type: userDetails[0]?.branch_type,
		}

		const creds_HeadOffice = {
			branch_id: branch_Select,
			tenant_id: userDetails[0]?.tenant_id,
			from_dt: recoveryStatus == 'A' ? fromDate : transactionDate,
			to_dt: recoveryStatus == 'A' ? toDate : transactionDate,
			approval_status: recoveryStatus,
		...
		await axios
			.post(`${url_bdccb}/recov/fetch_group_recovery_list`, userDetails[0]?.branch_type === 'H' ? creds_HeadOffice : creds, {
				headers: {
			...
			>
				<main className="px-4 h-auto my-10 mx-32">
							...
							<div className="w-full flex flex-row-reverse justify-between items-center mx-4">
								...
								{userDetails[0]?.branch_type != 'H' && (
									<button
										...
										onClick={() => {
											navigate(`/homebm/loan_branch_group-recovery`)
										}}
									>
										Recovery
									</button>
					...
					<LoanRecoverySubmitSHGListTable_BDCCB
						...
						title="Loan Recovery Of Group List"
						showSearch={false}
...
export default RecoverySubmitStatusGroupBranchBM_BDCCB
