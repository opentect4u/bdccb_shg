...
const reportTypeOptions = [
	...
	{ label: "Memberwise", value: "M" },
]

function OutstandingReportIndirectMain() {
	...
	// Option Type (Default: G for Groupwise, M for Memberwise)
	...
	// Search Handler for Indirect Outstanding Report
	const handleSearchReport = async () => {
		...
		const tokenValue = await getLocalStoreTokenDts(navigate)
		const apiEndpoint = `${url}/fetch_indirect_groupwise_outstanding_report`
		...
		try {
			...
			if (res?.data?.suc === 1 && res?.data?.msg && res.data.msg.length > 0) {
				...
				setMetadataDtls({
					...
					report_type: reportType === "G" ? "Indirect Groupwise Outstanding" : "Indirect Memberwise Outstanding",
				})
			} else {
				Message("error", res?.data?.msg || "No indirect loan outstanding records found for selected branch")
				setReportData([])
			}
		} catch (err) {
			console.error("Search indirect outstanding report error:", err)
			Message("error", "Error fetching indirect loan outstanding report")
			setReportData([])
		}
		setLoading(false)
	}
	...
	const handleExcelExport = async () => {
		...
		subTitleCell.value = isGroup
			...
			: "BRANCH MEMBERWISE LOAN OUTSTANDING REPORT (INDIRECT)"
		...
		filteredReportData.forEach((item, index) => {
			...
			if (isGroup) {
				...
				addedRow.getCell(17).numFmt = "#,##0.00"
				...
				addedRow.getCell(15).alignment = { horizontal: "center" }
				addedRow.getCell(16).alignment = { horizontal: "center" }
				addedRow.getCell(17).alignment = { horizontal: "right" }
			}
		})
		...
		const fileName = `Indirect_Loan_Outstanding_Report_${isGroup ? "Groupwise" : "Memberwise"}_${moment().format("YYYYMMDD_HHmmss")}.xlsx`
	...
	const handlePrintReport = () => {
		...
		const tableHeaderHtml = isGroup
			...
			: `
				<tr>
					...
					<th>Start Date</th>
					<th>End Date</th>
					<th style="text-align: right;">Member Outstanding (₹)</th>
				</tr>
			`
		...
		const tableRowsHtml = filteredReportData.map((item, index) => {
			...
			if (isGroup) {
				...
				const outstandingVal = Number(item?.member_outstanding || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })

				return `
					<tr>
						<td style="text-align: center;">${index + 1}</td>
						<td style="font-family: monospace;">${item?.loan_id || ""}</td>
						<td style="font-family: monospace;">${item?.ccb_loan || item?.loan_id || ""}</td>
						<td style="font-family: monospace;">${item?.group_code || ""}</td>
						<td style="font-weight: 600;">${item?.group_name || "N/A"}</td>
		...
		const htmlContent = `
			...
			<html>
			<head>
				<title>Loan Outstanding Report (Indirect)</title>
				<style>
					...
					.header h3 { margin: 4px 0 0 0; color: #334155; font-size: 14px; font-weight: 700; text-transform: uppercase; }
			...
			<body>
				<div class="header">
					<h3>${isGroup ? "BRANCH GROUPWISE LOAN OUTSTANDING REPORT (INDIRECT)" : "BRANCH MEMBERWISE LOAN OUTSTANDING REPORT (INDIRECT)"}</h3>
				</div>
				<div class="meta-info">
				...
				<table>
					...
					<tfoot>
						<tr>
							<td colSpan="${isGroup ? 8 : 12}" style="text-align: right; text-transform: uppercase;">Total Summary:</td>
				...
				<main className="w-[95%] max-w-[1400px] mx-auto my-6 p-6 md:p-8 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 transition-all duration-300">
						...
						<div className="flex items-center gap-4">
							...
							<div>
								<h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
									Loan Outstanding Report (Indirect)
								</h1>
								<p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
									View groupwise and memberwise indirect loan outstanding details as on date
								</p>
							</div>
						</div>
					</div>

					{/* Control / Filter Card (Sequence: Select Branch -> Radio Option -> As on Date -> Search) */}
					<div className="bg-slate-50/90 dark:bg-slate-800/60 p-5 md:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-700/60 shadow-sm mb-6">
						<div className="grid grid-cols-1 md:grid-cols-12 items-end gap-4">
							...
							{/* Select Option Radio Buttons (G / M - 3 cols) */}
							...
							<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-200 dark:border-slate-800">
								<div>
									<h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
										...
										<span>
											{isGroup ? "Indirect Groupwise Loan Outstanding Details" : "Indirect Memberwise Loan Outstanding Details"}
										</span>
									</h3>
									...
									<thead className="text-xs uppercase bg-slate-800 text-slate-100 dark:bg-slate-900 sticky top-0 z-10">
										{isGroup ? (
											...
											<tr>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider text-center">Sl. No.</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Loan ID</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">CCB Loan</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Group Code</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Group Name</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Member Code</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Member Name</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Society Name</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider text-center">Period</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider text-right">Curr ROI (%)</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider text-right">Penal ROI (%)</th>
												...
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">Start Date</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider">End Date</th>
												<th scope="col" className="px-4 py-3 font-bold tracking-wider text-right">Member Outstanding (₹)</th>
											</tr>
										)}
									</thead>
										...
										{currentData.map((item, i) => (
											<tr key={i} className="bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
												...
												<td className="px-4 py-3 text-right font-bold text-teal-600 dark:text-teal-400">
													{Number(isGroup ? (item?.group_outstanding || 0) : (item?.member_outstanding || 0)).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
												</td>
											</tr>
										))}
									</tbody>
...
export default OutstandingReportIndirectMain
