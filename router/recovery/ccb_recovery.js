...
ccb_recovRouter.post("/fetch_grp_dt", async (req, res) => {
  try {
    ...
    var mem_select = "acc_status",
      ...
      mem_whr = `tenant_id = '${tenant_id}' AND (loan_acc_no = '${ccb_loan_acc_no}' OR society_acc_no = '${ccb_loan_acc_no}')`,
      mem_order = null;
...
ccb_recovRouter.post("/fetch_loan_dtls_based_ccbacc_no", async (req, res) => {
  try {
    ...
    if (fetch_ccb_loan_dtls.suc === 1 && fetch_ccb_loan_dtls.msg.length > 0) {
      ...
      var select_mem_recov = "a.loan_id,a.branch_shg_id,b.member_code,c.member_name,a.ccb_loan_id,COALESCE(SUM(a.cr_amt),0) AS cr_amt,(COALESCE(b.disb_amt,0)) AS mem_disb_amt,(COALESCE(b.prn_amt,0)) AS mem_outstanding,(COALESCE(b.intt_amt,0)) AS mem_intt_outstanding",
        ...
        whr_mem_recov = `a.loan_acc_no = '${ccb_acc_no}' AND a.tenant_id = '${tenant_id}' AND a.branch_id = '${branch_id}' AND a.ccb_loan_id = '${ccb_loan_id}' AND a.approval_status = 'U' AND b.fund_type = 'B' GROUP BY a.loan_id,a.branch_shg_id,b.member_code,c.member_name,a.ccb_loan_id,b.disb_amt,b.prn_amt,b.intt_amt`,
...
//     const values2 = [date,ccb_trans_id,dt.loan_id,ccb_loan_id,tenant_id,branch_id,loan_to,branch_shg_id,loan_acc_no,'I',Number(dt.calculated_interest),0,0,0,0,0,Number(dt.curr_prn + dt.curr_intt + dt.calculated_interest),0,0,0,'U',created_by,datetime,ip_address];
//     const whereColumns2 = [];
//     const whereValues2 = [];
//     const flag2 = 0;
  ...
  try {
    ...
    for (const dt of ccb_recov) {
      ...
      if (Number(dt.amount) > 0) {
        ...
        let updated_prn = Number(dt.curr_prn) + Number(dt.curr_intt) + Number(dt.calculated_interest);
        let current_prn = updated_prn - Number(dt.prn_recov + dt.intt_recov);
        let current_intt_prn = Number(dt.calculated_interest) - Number(dt.intt_recov);
  ...
  try {
    ...
    var select = "a.branch_id,a.loan_id,a.group_code,d.group_name,a.loan_acc_no,a.period,a.curr_roi,a.penal_roi,TO_CHAR(a.disb_dt, 'YYYY-MM-DD') AS disb_dt,a.disb_amt,(a.curr_prn + a.curr_intt) AS loan_outstanding,CASE WHEN COALESCE(r.cr_amt,0) >= COALESCE(i.dr_amt,0) THEN COALESCE(r.curr_prn_recov,0) - COALESCE(i.dr_amt,0) ELSE 0 END AS principal_amount, CASE WHEN COALESCE(r.cr_amt,0) >= COALESCE(i.dr_amt,0) THEN COALESCE(i.dr_amt,0) ELSE COALESCE(r.cr_amt,0) END AS interest_amount",
     ...
     JOIN bdccb.md_group d ON a.group_code = d.group_code`,
      ...
      whr = `a.tenant_id = '${tenant_id}' AND a.branch_id = '${branch_id}' AND a.group_code = '${group_code}' ${loan_id ? `AND a.loan_id = '${loan_id}'` : ''}`,
      order = null;
    var fetch_ccb_loan_dtls1 = await db_Select(select, table_name, whr, order);
...
LEFT JOIN bdccb.td_loan_member_trans d 
 ...
 ON a.loan_id = i.loan_id 
AND a.ccb_loan_id = i.ccb_loan_id`;
      whr_member = `a.tenant_id = '${tenant_id}' AND a.branch_id = '${branch_id}' AND a.group_code = '${group_code}' ${loan_id ? `AND a.ccb_loan_id = '${loan_id}'` : ''}`;
      order_member = null;
      var fetch_member_dtls_trans1 = await db_Select(select_member, table_member, whr_member, order_member);
...
ccb_recovRouter.post("/accept_ccb_recovery", async (req, res) => {
  try {
    ...
    const interest_row_ccb = await db_Select(select_int, table_name_int, whr_int, order_int);
...
ccb_recovRouter.post("/fetch_society_loan_dtls", async (req, res) => {
  try {
    const { tenant_id, branch_code, group_code, society_acc_no, branch_type } = req.body;

    let branch_condition = '';

    if (branch_type === 'B') {
      branch_condition = `a.branch_id = '${branch_code}'`
    } else {
