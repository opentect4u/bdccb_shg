...
society_recovRouter.post("/fetch_soc_mem_dtls", async (req, res) => {
  ...
  var select = "a.loan_id,a.group_code,d.group_name,a.loan_acc_no,c.society_acc_no,a.period,a.curr_roi,a.penal_roi,TO_CHAR(a.disb_dt, 'YYYY-MM-DD') AS disb_dt,a.disb_amt,(a.curr_prn + a.curr_intt) AS loan_outstanding,(SUM(CASE WHEN b.trans_type = 'R' THEN COALESCE(b.curr_prn_recov,0) ELSE 0 END) - SUM(CASE WHEN b.trans_type = 'I' THEN COALESCE(b.dr_amt,0) ELSE 0 END)) AS principal_amount,SUM(CASE WHEN b.trans_type = 'I' THEN COALESCE(b.dr_amt,0) ELSE 0 END) AS interest_amount",
