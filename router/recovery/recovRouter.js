    ...
    // ***** INSERT/UPDATE GROUP LOAN TRANSACTION TABLE  *****
      ...
      for(meb of members) {
       let member_transid = meb.memb_trans_id > 0 ? meb.memb_trans_id : await transaction_id();
        var member_current_prn = meb.memb_curr_prn - meb.memb_recov_prin;
        ...
        const member_loan_columns = ["prn_amt","modified_by","modified_at","ip_address"];
        const member_loan_values = [member_current_prn,created_by,datetime,ip_address];
        const member_loan_whereColumns = ["loan_id","member_code"];
        const member_loan_whereValues = [meb.memb_loan_id,meb.member_code]; 
