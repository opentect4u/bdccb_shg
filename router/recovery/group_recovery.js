...
groupRecoveryRouter.post("/fetch_loan_details", async (req, res) => {
  try {
    ...
    var select = "DISTINCT a.group_code,a.group_name,b.loan_to",
      ...
      whr = `CAST(a.group_code AS text) = '${group_codes}' OR LOWER(a.group_name) LIKE LOWER('%${group_codes}%')`,
      order = null;
      ...
      if (fetch_loan_data.suc === 1 && fetch_loan_data.msg.length > 0) {
        for (let loan of fetch_loan_data.msg) {
          let mem_select = "a.loan_id AS mem_loan_id,a.group_code,c.group_name,a.member_code AS member_id,d.member_name,COALESCE(a.prn_amt,0) AS principal_amt,d.member_account_no AS sb_acc_no,e.approval_status",
            ...
            mem_whr = `a.tenant_id = '${tenant_id}' AND a.ccb_loan_id = '${loan.loan_id}' AND a.group_code = '${group_code}' AND b.approval_status = 'A' GROUP BY a.loan_id,a.group_code,c.group_name,a.member_code,d.member_name,a.prn_amt,d.member_account_no,e.approval_status`;
...
groupRecoveryRouter.post("/fetch_group_recovery_list", async (req, res) => {
  try {
    ...
    if (branch_type === 'B') {
      branch_condition = `c.branch_id = '${branch_id}'`;
    } else if (branch_type === 'P') {
      branch_condition = `c.branch_shg_id = '${branch_id}'`;
...
groupRecoveryRouter.post("/fetch_group_mem_recov_dtls", async (req, res) => {
  try {
    const { tenant_id, branch_id, group_code, trans_dt, approval_status } = req.body;
    ...
    var whr_loan = `a.tenant_id = '${tenant_id}' AND a.group_code = '${group_code}' GROUP BY a.group_code, b.group_name`;
    ...
    if (fetch_loan_dtls.suc === 1 && fetch_loan_dtls.msg.length > 0) {
      ...
      var whr_mem = `a.tenant_id = '${tenant_id}' AND a.group_code = '${group_code}'`;
