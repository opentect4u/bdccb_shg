...
groupRouter.post("/fetch_group_details", async (req, res) => {
 try{
  ...
  whr = `a.group_code IN (${groupCodes}) AND a.delete_flag = 'N'`,
  order = null;
  var fetch_group_data = await db_Select(select,table_name,whr,order);
...
groupRouter.post("/save_group", async (req, res) => {
    try {
      ...
      const table = "bdccb.md_group";
      const columns = group_code > 0 ? ["group_name","phone1","sahayika_id","group_addr","dist_id","block_id","ps_id","po_id","gp_id","village_id","pin_no","modified_by","modified_at","ip_address"] : ["group_code","branch_code","group_name","phone1","sahayika_id","group_addr","dist_id","block_id","ps_id","po_id","gp_id","village_id","pin_no","open_close_flag","grp_open_dt","delete_flag","created_by","created_at","ip_address","direct_indirect_flag","pacs_id"];
      const values = group_code > 0 ? [group_name || null,phone,sahayikaId,group_addr.replace(/'/g, "''"),distId,blockId,psId,poId,gpId,villageId,pin,created_by,datetime,ip_address] : [grp_code,branch_code,group_name,phone,sahayikaId,group_addr ? group_addr.replace(/'/g, "''") : null,distId,blockId,psId,poId,gpId,villageId,pin,'O',datetime,'N',created_by,datetime,ip_address,direct_indirect_flag,pacs_id];
